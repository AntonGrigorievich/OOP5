...
public:
    ...
    explicit Base(const string& material_input) : material(material_input) {
        id = ++totalObjects;
    ...
    explicit Base(Base* other) : material(other ? other->material : "") {
        id = ++totalObjects;
        cout << "Base(Base*): создан объект id=" << id
             << " из указателя на id=" << (other ? other->id : -1) << endl;
    }
...
public:
    ...
    Desc() : Base(), value(10.0) {
        descID = ++totalDesc;
        cout << "Desc(): создан потомок id=" << descID
             << ", значение=" << value << endl;
    }

    Desc(double val, const string& material_input) : Base(material_input), value(val) {
        descID = ++totalDesc;
    ...
    explicit Desc(Desc* other) : Base(other), value(other ? other->value : 0.0) {
        descID = ++totalDesc;
        cout << "Desc(Desc*): создан потомок id=" << descID
             << " из указателя на потомка id=" << (other ? other->descID : -1) << endl;
    }

    ~Desc() override {
        totalDesc--;
        cout << "~Desc(): удалён потомок id=" << descID
             << ", значение=" << value << endl;
    }
...
void demonstrateUniquePtr() {
    ...
    unique_ptr<Desc> ptr2 = std::move(ptr1);
